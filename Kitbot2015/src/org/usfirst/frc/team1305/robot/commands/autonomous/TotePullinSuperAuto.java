package org.usfirst.frc.team1305.robot.commands.autonomous;

import org.usfirst.frc.team1305.robot.commands.forks.OpenAndDeployForks;

import edu.wpi.first.wpilibj.command.CommandGroup;

/**
 * Autonomous which deploys forks, drives foreward
 */
public class TotePullinSuperAuto extends CommandGroup {
    
    public  TotePullinSuperAuto() {
        // Add Commands here:
        // e.g. addSequential(new Command1());
        //      addSequential(new Command2());
        // these will run in order.

        // To run multiple commands at the same time,
        // use addParallel()
        // e.g. addParallel(new Command1());
        //      addSequential(new Command2());
        // Command1 and Command2 will run in parallel.

        // A command group will require all of the subsystems that each member
        // would require.
        // e.g. if Command1 requires chassis, and Command2 requires arm,
        // a CommandGroup containing them would require both the chassis and the
        // arm.
    	
    	addSequential(new OpenAndDeployForks());
    	addSequential(new DriveWhileForksOpen());
    	addSequential(new Wait(0.5));
    	addSequential(new ElevatorUp(0.3));
    	addSequential(new AutonomousMobility(5, -0.3, -0.3));
    }
}
